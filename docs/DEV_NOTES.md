...
### v1.2 — Звук и вибрация во время тренировки — спроектировано, не реализовано

#### Концепция

Пользователь может тренироваться не глядя в телефон: аудио- и тактильные сигналы
информируют о ключевых моментах тренировки. Телефон лежит в стороне — сигнал
говорит «начинай» или «время вышло».

#### Ключевые моменты (тригеры)

| Момент | Звук | Вибрация |
|--------|------|----------|
| Таймер отдыха: последние 3 секунды | тихий тик × 3 | лёгкая × 3 |
| **Таймер отдыха истёк** → упражнение | **ding** (высокий) | **сильная × 1** |
| Тренируемое упражнение (timed): последние 3 сек | тихий тик × 3 | лёгкая × 3 |
| **Таймер упражнения истёк** | **ding** (средний) | **сильная × 1** |
| Пользователь нажал «Готово» (подход засчитан) | мягкий pop | средняя × 1 |
| **Тренировка завершена** | мелодия (3 ноты) | паттерн «трель» × 3 |

Последние 3 секунды таймера — общий паттерн для таймера отдыха и timed-упражнений.
Для reps-упражнений таймер отдыха та же механика, timed-упражнений — оба таймера.

#### Реализация

**Вибрация:** `HapticFeedback` из Flutter SDK (нет зависимостей):
- `HapticFeedback.lightImpact()` — тики обратного отсчёта
- `HapticFeedback.heavyImpact()` — конец таймера / завершение
- `HapticFeedback.mediumImpact()` — засчитанный подход
- `HapticFeedback.vibrate()` — трель (вызывается 3× с задержкой 150мс) для завершения тренировки

**Звук:** пакет `audioplayers` (или `just_audio`).
Короткие `.mp3` файлы в `assets/sounds/`:
- `tick.mp3` — ~50мс, тихий тик (отсчёт)
- `ding.mp3` — ~300мс, высокий сигнал (конец таймера)
- `pop.mp3` — ~100мс, мягкий pop (засчитан подход)
- `complete.mp3` — ~700мс, мелодия завершения (3 ноты)

Звуки должны быть тихими и ненавязчивыми. Можно взять из бесплатных библиотек
(freesound.org, mixkit.co) — лицензия CC0 или Mixkit License.

**Где вызывать в коде:**

| Место | Тригер |
|-------|--------|
| `_RestTimer` в WorkoutScreen | `onTick` при `remaining <= 3` → tick; `onFinish` → ding + heavy |
| `_TimedDisplay` (timed exercise) | `onTick` при `remaining <= 3` → tick; `onFinish` → ding + heavy |
| `WorkoutNotifier._completeSet` | после засчитывания → pop + medium |
| `WorkoutNotifier._finishWorkout` | → complete sound + vibrate pattern |

`_RestTimer` и `_TimedDisplay` уже используют `Timer.periodic` — добавить side-effect
на нужных тиках. Не создавать новый таймер.

#### Настройки

Два отдельных toggle в Settings (раздел «ТРЕНИРОВКА»):

```
ТРЕНИРОВКА
Звуковые сигналы    [  ●  ]   ← soundEnabled
Вибрация            [  ●  ]   ← hapticEnabled
```

Оба сохраняются в `SettingsNotifier` / `UserProfile` (или отдельно через SharedPreferences,
т.к. это device-настройка, а не профиль). Рекомендуется SharedPreferences или `UserProfile`
@HiveField поля — решить при реализации.

#### Технические задачи

| # | Задача |
|---|--------|
| 1 | `audioplayers` или `just_audio` в pubspec |
| 2 | 4 звуковых файла в `assets/sounds/`, зарегистрировать в pubspec.yaml |
| 3 | `SoundService` в `lib/core/services/` — singleton, методы `tick()`, `ding()`, `pop()`, `complete()` |
| 4 | `HapticService` (или методы в том же сервисе) — `light()`, `heavy()`, `medium()`, `pattern()` |
| 5 | `SettingsNotifier` + `UserProfile` / SharedPreferences: `soundEnabled`, `hapticEnabled` |
| 6 | Settings UI: два toggle в разделе «ТРЕНИРОВКА» |
| 7 | `_RestTimer` / `_TimedDisplay` в WorkoutScreen — side-effects на нужных тиках |
| 8 | `WorkoutNotifier` — сигналы при `_completeSet` и `_finishWorkout` |
| 9 | l10n: строки настроек |
